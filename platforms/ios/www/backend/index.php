...
/*$config['db']['user']   = "B1173_papiuser";
$config['db']['pass']   = "2QC6AnDj7J";
$config['db']['dbname'] = "B1173_persocoapi";*/
...
$app->get('/getCScode/{token}', function (Request $request, Response $response) {
    $token = $request->getAttribute('token');
    ...
    try {
        ...
        $response->write('{"code": "'.$participant->crowdsourcing_code.'"}');
        return $response;        
    } 
...
$app->post('/saveNotification', function (Request $request, Response $response) {
    ...
    try 
    {
        ...
        // Add the response to this participant
        $r_query = $this->db->prepare("INSERT INTO notifications(participant_id, usefulness_of_notification) VALUES(:pid, :uon)");
        $r_query->bindParam(':pid', $participant->participant_id);
